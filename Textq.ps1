...
$RawURL = "https://github.com/Hobrinelol/FederTweet-Tool/blob/main/Textq.ps1"
$ReleaseURL = "https://api.github.com/repos/Hobrinelol/FederTweet-Tool/commits/715ed9b3a015dcad3305a3758d576f33e6e49b80"

function Show-LoadingBar {
    param([string]$Message = "Initializing", [int]$Duration = 2000)
    $frames = @("|", "/", "-", "\")
...
function Check-AndUpdate {
    ...
    Write-Host "============================================================" -ForegroundColor DarkCyan
    try {
        $response = Invoke-RestMethod -Uri $ReleaseURL -Method Get -TimeoutSec 5
        ...
        for ($i = 0; $i -lt [math]::Min($currentParts.Count, $latestParts.Count); $i++) {
            if ($latestParts[$i] -gt $currentParts[$i]) { $updateAvailable = $true; break }
            elseif ($latestParts[$i] -lt $currentParts[$i]) { break }
        }
        if ($updateAvailable) {
            ...
            Show-LoadingBar -Message " Restarting" -Duration 1500
            Write-Host ""
            Write-Host "============================================================" -ForegroundColor DarkCyan
...
class FederTweetTool {
    ...
    [hashtable]$ServiceStatus = @{ "FederTweet API" = $false; "AI Service" = $false; "Last Check" = $null }
    ...
    [void] CheckServiceStatus() {
        Clear-Host; $this.ShowBanner()
        Write-Host ""; Write-Host ("=" * 60) -ForegroundColor DarkCyan
        ...
        try {
            ...
            $webRequest.Method = "GET"; $webRequest.Timeout = 5000
            $response = $webRequest.GetResponse()
            $this.ServiceStatus["FederTweet API"] = $true; $response.Close()
            Write-Host "`r FederTweet API: [ONLINE]" -ForegroundColor Green
        } catch { $this.ServiceStatus["FederTweet API"] = $false; Write-Host "`r FederTweet API: [OFFLINE]" -ForegroundColor Red }
        $this.ShowLoader(" Checking AI Service", 800)
        try {
            $aiCheck = $false; $this.ServiceStatus["AI Service"] = $aiCheck
            if ($aiCheck) { Write-Host "`r AI Service: [ONLINE]" -ForegroundColor Green }
            else { Write-Host "`r AI Service: [OFFLINE]" -ForegroundColor Yellow }
        } catch { $this.ServiceStatus["AI Service"] = $false; Write-Host "`r AI Service: [OFFLINE]" -ForegroundColor Red }
    ...
    [void] ShowServiceStatus() {
        Write-Host ""; Write-Host ("=" * 60) -ForegroundColor DarkCyan
        Write-Host " SERVICE STATUS" -ForegroundColor White; Write-Host ("=" * 60) -ForegroundColor DarkCyan
        Write-Host " FederTweet API: " -NoNewline
        if ($this.ServiceStatus["FederTweet API"]) { Write-Host "[ONLINE]" -ForegroundColor Green }
        else { Write-Host "[OFFLINE]" -ForegroundColor Red }
        Write-Host " AI Service: " -NoNewline
        if ($this.ServiceStatus["AI Service"]) { Write-Host "[ONLINE]" -ForegroundColor Green }
        else { Write-Host "[OFFLINE]" -ForegroundColor Yellow }
        if ($this.ServiceStatus["Last Check"]) { Write-Host ""; Write-Host " Last Check: $($this.ServiceStatus["Last Check"])" -ForegroundColor Gray }
        Write-Host ("-" * 60) -ForegroundColor DarkGray
        Write-Host " API Documentation: $($this.ApiDocsURL)" -ForegroundColor Gray
        Write-Host ("=" * 60) -ForegroundColor DarkCyan
    }
    ...
    [void] ShowLoader([string]$message, [int]$duration = 1000) {
        $frames = @("|", "/", "-", "\"); $endTime = (Get-Date).AddMilliseconds($duration); $i = 0
        while ((Get-Date) -lt $endTime) {
            Write-Host "`r$message $($frames[$i % 4])" -NoNewline -ForegroundColor Gray; $i++; Start-Sleep -Milliseconds 100
        }
        Write-Host "`r$message " -NoNewline; Write-Host "[" -NoNewline -ForegroundColor Green
        Write-Host "DONE" -NoNewline -ForegroundColor Green; Write-Host "]" -ForegroundColor Green
    }

    [void] ShowProgressBar([int]$current, [int]$total, [string]$message = "") {
        $width = 30; $percent = [math]::Round(($current / $total) * 100)
        $filled = [math]::Round(($width * $current) / $total); $empty = $width - $filled
        $bar = "[" + ("#" * $filled) + ("." * $empty) + "]"
        if ($percent -ge 70) { $color = "Green" } elseif ($percent -ge 30) { $color = "Yellow" } else { $color = "Red" }
        Write-Host "`r$message $bar $percent% " -NoNewline -ForegroundColor $color
    }

    [object] MakeRequest([string]$Endpoint) {
        if (-not $this.ServiceStatus["FederTweet API"]) { Write-Host ""; Write-Host "ERROR: FederTweet API is offline!" -ForegroundColor Red; return $null }
        ...
        try {
            if ($this.RateLimits.ContainsKey("Remaining") -and $this.RateLimits["Remaining"] -le 0) {
                ...
                Write-Host ""; Write-Host ("!" * 50) -ForegroundColor Red
                Write-Host " RATE LIMIT REACHED - Waiting for reset..." -ForegroundColor Red; Write-Host ("!" * 50) -ForegroundColor Red
                for ($i = $waitTime.TotalSeconds; $i -gt 0; $i--) {
                    $this.ShowProgressBar($waitTime.TotalSeconds - $i + 1, $waitTime.TotalSeconds, " Waiting:"); Start-Sleep -Seconds 1
                }
                Write-Host ""
            }
            $this.ShowLoader(" Sending request", 800)
            $webRequest = [System.Net.WebRequest]::Create($url); $webRequest.Method = "GET"
            $webRequest.UserAgent = "FederTweet-PowerShell-Tool/2.1"
            $response = $webRequest.GetResponse(); $stream = $response.GetResponseStream()
            $reader = New-Object System.IO.StreamReader($stream); $responseBody = $reader.ReadToEnd()
            $reader.Close(); $stream.Close(); $response.Close()
            if ($response.Headers["X-RateLimit-Limit"]) { $this.RateLimits["Limit"] = [int]$response.Headers["X-RateLimit-Limit"] }
            if ($response.Headers["X-RateLimit-Remaining"]) { $this.RateLimits["Remaining"] = [int]$response.Headers["X-RateLimit-Remaining"] }
            if ($response.Headers["X-RateLimit-Reset"]) { $this.RateLimits["Reset"] = [long]$response.Headers["X-RateLimit-Reset"] }
            return $responseBody | ConvertFrom-Json
        } catch {
            if ($_.Exception.Message -like "*429*") {
                Write-Host ""; Write-Host ("!" * 50) -ForegroundColor Red
                Write-Host " RATE LIMIT EXCEEDED - Auto-waiting 60s..." -ForegroundColor Red; Write-Host ("!" * 50) -ForegroundColor Red
                for ($i = 60; $i -gt 0; $i--) { $this.ShowProgressBar(60 - $i + 1, 60, " Countdown:"); Start-Sleep -Seconds 1 }
                Write-Host ""; return $this.MakeRequest($Endpoint)
            } else {
                Write-Host ""; Write-Host ("!" * 50) -ForegroundColor Red
                Write-Host " ERROR: $($_.Exception.Message)" -ForegroundColor Red; Write-Host ("!" * 50) -ForegroundColor Red
                return $null
            }
        }
    }
    ...
    [void] DisplayRateLimit() {
        if ($this.RateLimits.ContainsKey("Remaining") -and $this.RateLimits.ContainsKey("Limit")) {
            ...
            Write-Host " RATE LIMIT STATUS" -ForegroundColor White; Write-Host ("-" * 60) -ForegroundColor DarkGray
            $this.ShowProgressBar($this.RateLimits["Remaining"], $this.RateLimits["Limit"], " Remaining:"); Write-Host ""
    ...
    [void] GetUserTweets([string]$Username) {
        Clear-Host; $this.ShowBanner()
        Write-Host ""; Write-Host ("=" * 60) -ForegroundColor DarkCyan
        Write-Host " FETCHING TWEETS FOR: @$Username" -ForegroundColor White; Write-Host ("=" * 60) -ForegroundColor DarkCyan
        $response = $this.MakeRequest("/user/$Username/tweets")
        if ($response -and $response.success) {
            $tweets = $response.tweets
            if ($tweets.Count -eq 0) { Write-Host ""; Write-Host "No tweets found for @$Username" -ForegroundColor Yellow; return }
            ...
            Write-Host ""; Write-Host "STATISTICS:" -ForegroundColor Cyan
            ...
            Write-Host ""; Write-Host "LATEST TWEETS:" -ForegroundColor Cyan; Write-Host ("-" * 60) -ForegroundColor DarkGray
            $maxTweets = [math]::Min(2, $tweets.Count-1)
            if ($maxTweets -ge 0) { foreach ($tweet in $tweets[0..$maxTweets]) { $this.DisplayTweet($tweet); Start-Sleep -Milliseconds 300 } }
    ...
    [void] GetGlobalFeed() {
        Clear-Host; $this.ShowBanner()
        Write-Host ""; Write-Host ("=" * 60) -ForegroundColor DarkCyan
        Write-Host " GLOBAL FEED" -ForegroundColor White; Write-Host ("=" * 60) -ForegroundColor DarkCyan
        $response = $this.MakeRequest("/tweets")
        if ($response -and $response.success) {
            $tweets = $response.tweets
            if ($tweets.Count -eq 0) { Write-Host ""; Write-Host "No tweets in feed" -ForegroundColor Yellow; return }
            ...
            Write-Host ""; Write-Host "TOP LIKED:" -ForegroundColor Yellow
            foreach ($tweet in $topLiked) { Write-Host "  @$($tweet.username): $($tweet.likes) likes" -ForegroundColor White }
            Write-Host ""; Write-Host "TOP RETWEETED:" -ForegroundColor Yellow
            foreach ($tweet in $topRetweeted) { Write-Host "  @$($tweet.username): $($tweet.retweets) retweets" -ForegroundColor White }
            Write-Host ""; Write-Host "RECENT TWEETS:" -ForegroundColor Cyan; Write-Host ("-" * 60) -ForegroundColor DarkGray
            $maxTweets = [math]::Min(2, $tweets.Count-1)
            if ($maxTweets -ge 0) { foreach ($tweet in $tweets[0..$maxTweets]) { $this.DisplayTweet($tweet); Start-Sleep -Milliseconds 300 } }
            $this.TweetCache.AddRange($tweets)
        }
    ...
    [void] GetUserProfile([string]$Username) {
        Clear-Host; $this.ShowBanner()
        Write-Host ""; Write-Host ("=" * 60) -ForegroundColor DarkCyan
        Write-Host " USER PROFILE: @$Username" -ForegroundColor White; Write-Host ("=" * 60) -ForegroundColor DarkCyan
        $response = $this.MakeRequest("/user/$Username")
        if ($response -and $response.success) {
            $user = $response.user
            Write-Host ""; Write-Host "USER INFORMATION:" -ForegroundColor Cyan
            ...
            if ($user.partnerInfo -and $user.partnerInfo.username) {
                Write-Host ""; Write-Host "PARTNER:" -ForegroundColor Cyan
    ...
    [void] AnalyzeWithAI() {
        if (-not $this.ServiceStatus["AI Service"]) {
            ...
            $this.AnalyzeTweets(); return
        }
        if ($this.TweetCache.Count -eq 0) {
            Write-Host ""; Write-Host ("!" * 50) -ForegroundColor Red
            Write-Host " No tweets in cache for analysis!" -ForegroundColor Red
            Write-Host ("!" * 50) -ForegroundColor Red; return
        }
        Write-Host ""; Write-Host ("=" * 60) -ForegroundColor DarkCyan
        Write-Host " AI-POWERED ANALYSIS" -ForegroundColor White; Write-Host ("=" * 60) -ForegroundColor DarkCyan
        ...
        foreach ($tweet in $tweets) {
            ...
            if ($positive -gt $negative) { $positiveCount++ } elseif ($negative -gt $positive) { $negativeCount++ } else { $neutralCount++ }
        }
        Write-Host ""; Write-Host "ENGAGEMENT METRICS:" -ForegroundColor Cyan
        ...
        Write-Host ""; Write-Host "SENTIMENT ANALYSIS:" -ForegroundColor Cyan
        ...
        Write-Host ""; Write-Host "BEST PERFORMING:" -ForegroundColor Cyan
        ...
        Write-Host ""; Write-Host "WORST PERFORMING:" -ForegroundColor Cyan
        ...
        Write-Host ""; Write-Host "AI INSIGHTS:" -ForegroundColor Magenta
        if ($positiveCount -gt $negativeCount) { Write-Host "  Overall positive sentiment detected" -ForegroundColor Green }
        elseif ($negativeCount -gt $positiveCount) { Write-Host "  Overall negative sentiment detected" -ForegroundColor Red }
        else { Write-Host "  Neutral sentiment detected" -ForegroundColor Gray }
        if ($avgEngagement -gt 50) { Write-Host "  High engagement rate - Content resonates well" -ForegroundColor Green }
        elseif ($avgEngagement -gt 20) { Write-Host "  Moderate engagement rate - Room for improvement" -ForegroundColor Yellow }
        else { Write-Host "  Low engagement rate - Consider content strategy" -ForegroundColor Red }
    }
    ...
    [void] AnalyzeTweets() {
        ...
        Write-Host ""; Write-Host ("=" * 60) -ForegroundColor DarkCyan
        Write-Host " BASIC ANALYSIS" -ForegroundColor White; Write-Host ("=" * 60) -ForegroundColor DarkCyan
        $tweets = $this.TweetCache
        $totalLikes = ($tweets | Measure-Object -Property likes -Sum).Sum
        $totalRetweets = ($tweets | Measure-Object -Property retweets -Sum).Sum
        $totalEngagement = $totalLikes + $totalRetweets
        ...
        Write-Host "  Total Engagement: $totalEngagement" -ForegroundColor Green
        Write-Host "  Avg per Tweet:    $avgEngagement" -ForegroundColor Yellow
        Write-Host ""; Write-Host "BEST TWEET:" -ForegroundColor Cyan
        Write-Host "  Text:       $($bestTweet.text)" -ForegroundColor White
    ...
    [void] ShowStartMenu() {
        Clear-Host; $this.ShowBanner()
        Write-Host ""; Write-Host ("=" * 60) -ForegroundColor DarkCyan
        Write-Host " SERVICE SELECTION" -ForegroundColor White; Write-Host ("=" * 60) -ForegroundColor DarkCyan
        ...
        switch ($choice) {
            "1" { $this.CheckServiceStatus(); $this.WaitForKey(); $this.ShowStartMenu() }
            "2" { $this.ShowMenu() }
            "3" { Write-Host ""; Write-Host "API Documentation: $($this.ApiDocsURL)" -ForegroundColor Cyan; $this.WaitForKey(); $this.ShowStartMenu() }
            "0" { Write-Host ""; Write-Host "Exiting..." -ForegroundColor DarkCyan; exit }
            default { Write-Host ""; Write-Host "Invalid selection!" -ForegroundColor Red; $this.WaitForKey(); $this.ShowStartMenu() }
        }
    }
    ...
    [void] ShowMenu() {
        $choice = ""
        do {
            Clear-Host; $this.ShowBanner(); $this.ShowServiceStatus()
            Write-Host ""; Write-Host ("+" + ("-" * 58) + "+") -ForegroundColor DarkCyan
            ...
            $choice = Read-Host "`n> Selection"
            switch ($choice) {
                "1" { $username = Read-Host " Username"; $this.GetUserTweets($username); $this.WaitForKey() }
                "2" { $this.GetGlobalFeed(); $this.WaitForKey() }
                "3" { $username = Read-Host " Username"; $this.GetUserProfile($username); $this.WaitForKey() }
                "4" { $this.AnalyzeWithAI(); $this.WaitForKey() }
                "5" { $this.AnalyzeTweets(); $this.WaitForKey() }
                "6" { $this.DisplayRateLimit(); $this.WaitForKey() }
                "7" { $this.ShowLoader(" Clearing cache", 500); $this.TweetCache.Clear(); Write-Host ""; Write-Host "Cache cleared successfully!" -ForegroundColor Green; $this.WaitForKey() }
                "8" { $this.CheckServiceStatus(); $this.WaitForKey() }
                "0" { Write-Host ""; Write-Host "Exiting FederTweet Tool..." -ForegroundColor DarkCyan; break }
                default { Write-Host ""; Write-Host ("!" * 50) -ForegroundColor Red; Write-Host " Invalid selection!" -ForegroundColor Red; Write-Host ("!" * 50) -ForegroundColor Red; $this.WaitForKey() }
            }
        } while ($choice -ne "0")
    }
}
